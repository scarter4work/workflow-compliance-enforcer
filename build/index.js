...
function loadWorkflowState(issueNumber) {
...
function recordStep(name, status, duration, output) {
    workflow.steps.push({
        name,
        status,
...
function visualizeWorkflow(currentState) {
    ...
    switch (name) {
        case 'workflow_config': {
            ...
            if (action === 'view') {
                ...
                const newConfig = { ...config, ...args.config };
        ...
        case 'workflow_start_issue': {
            ...
            try {
                const issueData = execSync(`gh issue view ${issueNumber} --json state,title`, { encoding: 'utf-8', cwd: process.cwd(), timeout: 30000 });
        ...
        case 'workflow_run_tests': {
            ...
            const config = loadConfig();
            const testCommand = args.test_command || config.test_command || 'npm test';
        ...
        case 'workflow_verify_prod': {
            ...
            const testCommand = args.test_command || config.production_test_command || 'npm run test:prod';
            try {
                const output = execSync(testCommand, {
                    encoding: 'utf-8',
                    cwd: process.cwd(),
        ...
        case 'workflow_close_issue': {
            ...
            const saveAuditReport = args?.save_audit_report !== false;
            try {
                // Generate audit report
                ...
                // Close GitHub issue
                execSync(`gh issue close ${workflow.activeIssue} ` +
                    `-c "✅ Fixed and verified in production (${workflow.commitHash?.substring(0, 7)})\n\nWorkflow completed successfully. See audit report for details."`, { encoding: 'utf-8', cwd: process.cwd(), timeout: 30000 });
